    ...
    class TGM_Plugin_Activation {
        ...
        protected function do_plugin_install() {
            ...
            if (isset($_GET['plugin']) && ( isset($_GET['tgmpa-install']) && 'install-plugin' == $_GET['tgmpa-install'] )) {
                ...
                $title = esc_attr(sprintf($this->strings['installing'], $plugin['name']));
                ...
                if ($this->is_automatic) {
                    ...
                    if (is_wp_error($activate)) {
                        ...
                        echo '<p><a href="' . add_query_arg('page', $this->menu, network_admin_url('themes.php')) . '" title="' . esc_attr($this->strings['return']) . '" target="_parent">' . esc_attr($this->strings['return']) . '</a></p>';
                ...
                foreach ($message as $type => $plugin_groups) {
                    ...
                    foreach ($plugin_groups as $plugin_group_single_name) {
                        ...
                        if ($external_url && preg_match('|^http(s)?://|', $external_url)) {
                            $linked_plugin_groups[] = '<a href="' . esc_url($external_url) . '" title="' . esc_attr($plugin_group_single_name) . '" target="_blank">' . esc_attr($plugin_group_single_name) . '</a>';
    ...
    class TGMPA_List_Table extends WP_List_Table {
        ...
        protected function _gather_plugin_data() {
            ...
            foreach (TGM_Plugin_Activation::$instance->plugins as $plugin) {
                ...
                if ($external_url && preg_match('|^http(s)?://|', $external_url)) {
                    $table_data[$i]['plugin'] = '<strong><a href="' . esc_url($external_url) . '" title="' . esc_attr($plugin['name']) . '" target="_blank">' . esc_attr($plugin['name']) . '</a></strong>';
                } elseif (!$source || preg_match('|^http://wordpress.org/extend/plugins/|', $source)) {
                    $url = add_query_arg(
                            array(
                        'tab' => 'plugin-information',
                    ...
                    $table_data[$i]['plugin'] = '<strong><a href="' . esc_url($url) . '" class="thickbox" title="' . esc_attr($plugin['name']) . '">' . esc_attr($plugin['name']) . '</a></strong>';
                } else {
                    $table_data[$i]['plugin'] = '<strong>' . esc_attr($plugin['name']) . '</strong>'; // No hyperlink.
                }
        ...
        public function column_cb($item) {
            ...
            $value = esc_url($item['file_path']) . ',' . esc_url($item['url']) . ',' . esc_attr($item['sanitized_plugin']);
        ...
        public function process_bulk_actions() {
            ...
            if ('tgmpa-bulk-activate' === $this->current_action()) {
                ...
                $imploded = empty($plugin_names) ? '<strong>' . esc_attr($last_plugin) . '</strong>' : '<strong>' . ( implode(', ', esc_attr($plugin_names)) . '</strong> and <strong>' . esc_attr($last_plugin) . '</strong>.' );
    ...
    if (!class_exists('TGM_Bulk_Installer_Skin')) {
        ...
        class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
            ...
            public function bulk_footer() {
                ...
                foreach (TGM_Plugin_Activation::$instance->plugins as $plugin) {
                    if (!is_plugin_active($plugin['file_path'])) {
                        echo '<p><a href="' . add_query_arg('page', TGM_Plugin_Activation::$instance->menu, network_admin_url('themes.php')) . '" title="' . esc_attr(TGM_Plugin_Activation::$instance->strings['return']) . '" target="_parent">' . esc_attr(TGM_Plugin_Activation::$instance->strings['return']) . '</a></p>';
                        $complete[] = $plugin;
                        break;
                    }
                    // Nothing to store.
